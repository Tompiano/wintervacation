...
func InsertComment(m model.Comment) (err error) {
	result, err := DB.Exec("insert into comments(userID,productID,parentID,content)value(?,?,?,?)",
...
func UpdateComment(commentID int, content string) (err error) {
	_, err = DB.Exec("update comments set content=? where commentID=? ", content, commentID)
	if err != nil {
